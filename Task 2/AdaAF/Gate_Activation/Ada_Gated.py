...
class AdaDQNAFGated(nn.Module):
    def __init__(self, num_nodes, n_observations, n_actions):
        super(AdaDQNAFGated, self).__init__()
    ...
    def forward(self, x):
        ...
        # Compute activations
        relu_activation = F.relu(z)
        tanh_activation = F.tanh(z)

        # Gating mechanism
        gate = torch.sigmoid(self.gate * z)

        # Combine activations adaptively
        combined_activation = gate * relu_activation + (1 - gate) * tanh_activation

        # Apply to second layer
        z = self.layer2(combined_activation)
        relu_activation = F.relu(z)
        tanh_activation = F.tanh(z)
        ...
        return self.layer3(combined_activation)

# === Models ==================================
# --- DQN model 1 -  MAF, Tanh & PRelu
# Define the DQN model with a mixture of adaptive GCU and SQU activations
def gcu_activation(x):
    return x * torch.cos(x)


# Define the enhanced SQU activation function
def enhanced_squ_activation(x, alpha=0.1):
    return alpha * (x ** 2) + x


# Define the DQN model with adaptive GCU and SQU activations
class AdaDQNOAFGated(nn.Module):
    def __init__(self, num_nodes, n_observations, n_actions, alpha=0.1):
        super(AdaDQNOAFGated, self).__init__()
        # Shared linear layers
        self.layer1 = nn.Linear(n_observations, num_nodes)
        self.layer2 = nn.Linear(num_nodes, num_nodes)
        ...
        # Learnable weights for gating
        self.gate = nn.Parameter(torch.randn(num_nodes))
        self.alpha = alpha  # Parameter for SQU
    ...
    def forward(self, x):
        ...
        # Compute activations
        gcu_activation_output = gcu_activation(z)
        squ_activation_output = enhanced_squ_activation(z, self.alpha)

        # Gating mechanism
        gate = torch.sigmoid(self.gate * z)

        # Combine activations adaptively
        combined_activation = gate * gcu_activation_output + (1 - gate) * squ_activation_output
...
class AdaDQNAFOAFGated(nn.Module):
    def __init__(self, num_nodes, n_observations, n_actions):
        super(AdaDQNAFOAFGated, self).__init__()
        # Shared linear layers
        self.layer1 = nn.Linear(n_observations, num_nodes)
        self.layer2 = nn.Linear(num_nodes, num_nodes)
    ...
    def forward(self, x):
        ...
        # Compute activations
        gcu_activation_output = gcu_activation(z)
        tanh_activation_output = torch.tanh(z)

        # Gating mechanism
        gate = torch.sigmoid(self.gate * z)

        # Combine activations adaptively
        combined_activation = gate * gcu_activation_output + (1 - gate) * tanh_activation_output
...
def run_experiments():
    ...
    for trial_i in range(p_exper['num_trials']):
        ...
        policy_net = AdaDQNAFGated(p_exper['num_nodes'], n_observations, n_actions).to(device)
        target_net = AdaDQNAFGated(p_exper['num_nodes'], n_observations, n_actions).to(device)
    ...
    for trial_i in range(p_exper['num_trials']):
        ...
        policy_net = AdaDQNOAFGated(p_exper['num_nodes'], n_observations, n_actions).to(device)
        target_net = AdaDQNOAFGated(p_exper['num_nodes'], n_observations, n_actions).to(device)
        episode_durations, episode_rewards, episode_aux_rewards = run_exper_trial(policy_net, target_net, p_exper)
    ...
    for trial_i in range(p_exper['num_trials']):
        ...
        policy_net = AdaDQNAFOAFGated(p_exper['num_nodes'], n_observations, n_actions).to(device)
        target_net = AdaDQNAFOAFGated(p_exper['num_nodes'], n_observations, n_actions).to(device)
        episode_durations, episode_rewards, episode_aux_rewards = run_exper_trial(policy_net, target_net, p_exper)
    ...
    # result = mannwhitneyu(mean_rewards[1, :], alternative='greater')
    #
    ...
    #     print(
    #         f"The mean reward for tanh is statistically significantly greater than the mean reward for sigmoid at alpha={alpha}.")
    # else:
    #     print(
    #         f"The mean reward for tanh is NOT statistically significantly greater than the mean reward for sigmoid at alpha={alpha}.")
    ...
    a_title = 'Condition {0} - Ada_GatedAF'.format(ci + 1)
    ...
    performs['mean_cumul_tr'] = a_mean_cumul_tot_rewards
    performs['mean_cumul_ar'] = a_mean_cumul_aux_rewards
    a_title = 'Condition {0} - Ada_GatedOAF'.format(ci + 1)
    plot_durations_4(mean_durations[ci, :], std_durations[ci, :], mean_aux_rewards[ci, :], std_aux_rewards[ci, :],
                     performs, a_title)
    ...
    performs['mean_cumul_tr'] = a_mean_cumul_tot_rewards
    performs['mean_cumul_ar'] = a_mean_cumul_aux_rewards
    a_title = 'Condition {0} - Ada_GatedAFOAF'.format(ci + 1)
    plot_durations_4(mean_durations[ci, :], std_durations[ci, :], mean_aux_rewards[ci, :], std_aux_rewards[ci, :],
                     performs, a_title)
    ...
    torch.save(best_policy_nets[1], fn3)
...
def render_test_net():
    ...
    policy_net = AdaDQNAFGated(p_exper['num_nodes'], n_observations, n_actions)
