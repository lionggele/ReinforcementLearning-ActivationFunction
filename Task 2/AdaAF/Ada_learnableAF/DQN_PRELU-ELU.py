...
def run_experiments():
    ...
    if result.pvalue < alpha:
        print(
            f"The mean reward for PRELU is statistically significantly greater than the mean reward for ELU at alpha={alpha}.")
    else:
        print(
            f"The mean reward for PRELU is NOT statistically significantly greater than the mean reward for ELU at "
            f"alpha={alpha}.")
    ...
    a_title = 'Condition {0} - PRELU'.format(ci + 1)
    ...
    performs['mean_cumul_tr'] = a_mean_cumul_tot_rewards
    performs['mean_cumul_ar'] = a_mean_cumul_aux_rewards
    a_title = 'Condition {0} - ELU'.format(ci + 1)
    plot_durations_4(mean_durations[ci, :], std_durations[ci, :], mean_aux_rewards[ci, :], std_aux_rewards[ci, :],
                     performs, a_title)
